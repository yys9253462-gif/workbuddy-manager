"""密钥列表「列与表头对齐」的界面验收（issue #68）——真服务 + 真浏览器。
...
数据落在 dev/.keys-layout/（已 gitignore），截图在 dev/.shots-keys-layout/。
"""
...
DATA = REPO / 'dev' / '.keys-layout'
SHOTS = REPO / 'dev' / '.shots-keys-layout'
UPSTREAM_PORT = 7971
MANAGER_PORT = 7972
ADMIN_PW = 'keys-layout-pass'
UID = 'kl000000-0000-0000-0000-000000000010'
...
def seed_keys() -> None:
    """两把属性差异明显的密钥：只限国际版且有 IP 上限 / 不限版本不限 IP。

    差异要够大，任何两列互换都能被看出（例如「永不过期」不会出现在版本列、
    「国际版」不会出现在有效期列）。
    """
    sys.path.insert(0, str(REPO))
    os.environ.update({'WB_DB': str(DATA / 'manager.db'),
                       'WB_AUTH_DIR': str(DATA / 'auths')})
    from server import config, keysvc
    ...
    db.connect()
    base = time.time()
    k1 = keysvc.create_key('国际版-两模型', realm='global',
                           models=['global:gpt-5.6-sol', 'glm-5.2'],
                           max_ips=3, expires_at=int(base) + 86400 * 30)
    k2 = keysvc.create_key('不限定-长期', realm='', models=[],
                           max_ips=0, expires_at=None)
    print('已造密钥:', k1['name'], '/', k2['name'])
    db._conn.close()
    db._conn = None
...
def main() -> int:
    ...
    try:
        ...
        r = subprocess.run(['node', 'dev/verify_keys_layout.mjs'], cwd=str(REPO),
